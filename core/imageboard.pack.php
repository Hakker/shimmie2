			...
			foreach($tags as $tag) {
				...
				if(empty($id)) {
					...
					$database->execute(
							...
							array("id"=>$this->id, "tag"=>$ttpe->term));
				}
				else {
	...
	private static function build_ugly_search_querylet($terms) {
		...
		// one positive tag (a common case), do an optimised search
			...
			$query = new Querylet(
				...
				"
					SELECT DISTINCT images.*, UNIX_TIMESTAMP(posted) AS posted_timestamp
				...
				",
				str_replace(')','\)',str_replace('(','\(',$tag_search->variables)) );
		...
		// more than one positive tag, and zero or more negative tags
			...
			foreach($tag_search->variables as $tag) {
				$tag_ids = $database->get_col("SELECT DISTINCT id FROM tags WHERE tag LIKE :tag", array("tag"=>str_replace(')','\)',str_replace('(','\(',$tag)) ));
